...
L = sum(point_sources[2:len(point_sources)])*r*v*dx(0) - float(problem.boundary_condition_str)*(p_coeff*r*r + F_2_coeff)*r*v*dx(1)

u = Function(V)
solve(a == L, u, bc)
u = fu.level_u(u=u, const=-float(problem.boundary_condition_str))
